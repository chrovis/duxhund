...
(defn generate [argv]
  (when-let [{:keys [target bam sorted-bam output] :as opts}
             (get-opts! (parse-opts argv generate-opts))]
    (dux/generate bam sorted-bam target output opts)
    0))
...
(defn fixup [argv]
  (when-let [{:keys [input cache output]}
             (get-opts! (parse-opts argv fixup-opts))]
    (dux/fixup input cache output)
    0))

(def sub-commands
  [["generate" "Generate FASTQ for realignment from BWA alignment result."]
   ["fixup" "Fixup given SAM to make it suitable for fusionfusion call."]])
...
(defn -main [& argv]
  (let [[_ cmd sub-args help]
        ...
        ret (case cmd
              :generate (generate sub-args)
              :fixup (fixup sub-args)
              (println help))]
    (shutdown-agents)
